...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    IF v_member_id IS NOT NULL THEN
      -- A member may already be on the register (added manually or by an
      -- earlier booking). Adopt that row rather than raising a unique
      -- violation, which would abort the whole booking insert.
      ...
      ON CONFLICT (meeting_id, member_id) WHERE member_id IS NOT NULL
      DO UPDATE SET
        attendance_status = EXCLUDED.attendance_status,
        payment_method    = EXCLUDED.payment_method,
        amount_pence      = EXCLUDED.amount_pence,
        is_meeting_only   = EXCLUDED.is_meeting_only,
