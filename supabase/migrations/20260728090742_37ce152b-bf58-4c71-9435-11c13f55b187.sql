...
DECLARE
  ...
  v_ref_date date;
BEGIN
  -- Authorisation: only the member themselves, admins, secretary, WM, or the
  -- current-year Treasurer may look up another member's dues calculation
  -- (which exposes DOB-derived flags and exemption reasons).
...
-- last_engagement_date: restrict to the member themselves or a mentor/officer.
...
BEGIN
  IF _member <> auth.uid()
     AND NOT public.can_edit_member_development(auth.uid(), _member)
  THEN
    RAISE EXCEPTION 'not authorised';
  END IF;
