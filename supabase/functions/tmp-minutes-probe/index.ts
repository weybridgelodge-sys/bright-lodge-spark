...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let transcript_text: string = (body.transcript_text ?? "").toString();
    if (!transcript_text) {
      const { data: t } = await admin.from("meeting_minutes").select("transcript_text").eq("id", EXEMPLAR_ID).maybeSingle();
      transcript_text = (t?.transcript_text ?? "").toString();
    }
