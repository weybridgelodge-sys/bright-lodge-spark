...
async function fetchImageAsDataUrl(url: string): Promise<string | null> {
  try {
    ...
    // Lovable CDN asset paths ("/__l5e/...") are only served by Lovable's
    // hosting layer; the dev server, preview sandbox and native shell return
    // index.html for them, which silently produced blank cover images.
    const abs = assetUrl(url) || url;
    const res = await fetch(abs);
    if (!res.ok) return null;
    const blob = await res.blob();
    // Guard against an HTML fallback being embedded as a broken image.
