export type WorshipfulMasterEntry = { year: string; name: string; honours: string };

export const masters: WorshipfulMasterEntry[] = [
  ...
  { year: "2018", name: "Ben C Connolly", honours: "ProvAGDC" },
  { year: "2019", name: "Ben C Connolly", honours: "ProvAGDC" },
  { year: "2020", name: "Ben C Connolly", honours: "ProvAGDC" },
...
/**
 * Returns the earliest year (Oct installation) a member by name was installed as
 * Worshipful Master, by fuzzy-matching first-name + last-name against the
 * roll. Returns null if no match.
 */
export function firstWmYearForMember(
  ...
  const fnInitial = fn[0];
  let earliest: number | null = null;
  for (const m of masters) {
    const parts = m.name.trim().split(/\s+/);
    const surname = parts[parts.length - 1].toLowerCase();
    const firstWord = parts[0].toLowerCase().replace(/\./g, "");
    if (surname !== ln) continue;
    // Match if first name matches exactly, or the roll only has initials
    const firstMatches =
      firstWord === fn ||
      firstWord.startsWith(fn) ||
      fn.startsWith(firstWord) ||
      firstWord[0] === fnInitial;
