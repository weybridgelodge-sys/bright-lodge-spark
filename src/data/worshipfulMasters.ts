export type MasterEntry = { year: string; name: string; honours: string };

export const mastersRoll: MasterEntry[] = [
...
/**
 * Returns the earliest year in which a person matching the given first/last
 * name became Worshipful Master, or null. Matches by last-name (case-insensitive)
 * and the first initial of the first name, so "John T Coleman" in the roll
 * matches a member with first_name "John" last_name "Coleman".
 */
export function firstWmYearByName(firstName: string | null, lastName: string | null): number | null {
  if (!lastName) return null;
  const lastLower = lastName.trim().toLowerCase();
  const firstInitial = (firstName ?? "").trim().charAt(0).toLowerCase();
  let earliest: number | null = null;
  for (const m of mastersRoll) {
    const parts = m.name.trim().split(/\s+/);
    if (parts.length < 2) continue;
    const rollLast = parts[parts.length - 1].toLowerCase();
    const rollFirstInitial = parts[0].charAt(0).toLowerCase();
    if (rollLast !== lastLower) continue;
    if (firstInitial && rollFirstInitial !== firstInitial) continue;
